...
h52w = pd.read_excel('ALL_52WH_READY2.xlsx', index_col=0)  # set stock code as index
...
weight_h52w = (0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8)
combination_array = []

for w_l_uebit in weight_long_uebit:
    for w_s_uebit in weight_short_uebit:
        for w_h52w in weight_h52w:
            for i in range(len(rtr_array)):
                combination_array.append([w_l_uebit, w_s_uebit, w_h52w, rtr_array[i], rtr_array_str[i]])

def createDataFrame(quarter, w_leg, w_h52w, true_if_long):
    ...
    for ticker in uebit_leg_df_tickers_cut_off:
        ...
        uebit_leg_df_h52w_cut_off.append(h52w[quarter][ticker])
    uebit_leg_df["Ticker"] = uebit_leg_df_tickers_cut_off
    uebit_leg_df["uebit"] = uebit_leg_df_uebit_cut_off
    uebit_leg_df["h52w"] = uebit_leg_df_h52w_cut_off
    uebit_leg_df["beta"] = uebit_leg_df_beta_cut_off
    uebit_leg_df.set_index("Ticker")

    # END OF UEBIT DF
    # START OF UEBIT h52w DF

    uebit_h52w_leg_df = pd.DataFrame()
    uebit_h52w_leg_df_tickers = []
    uebit_h52w_leg_df_h52w = []

    uebit_h52w_leg_df_sorted = uebit_leg_df.sort_values(by="h52w", ascending=true_if_long)
    uebit_h52w_leg_df_sorted.reset_index(drop=True, inplace=True)

    # GET RID OF NAN
    for i in range(len(uebit_h52w_leg_df_sorted["Ticker"])):
        if uebit_h52w_leg_df_sorted["h52w"][i] > 0 or uebit_h52w_leg_df_sorted["h52w"][i] < 0:
            uebit_h52w_leg_df_tickers.append(uebit_h52w_leg_df_sorted["Ticker"][i])
            uebit_h52w_leg_df_h52w.append(uebit_h52w_leg_df_sorted["h52w"][i])

    # IDENTIFY CUTOFF POINT
    if true_if_long:
        cut_off_h52w = math.floor(w_h52w * len(uebit_h52w_leg_df_tickers))
    else:
        cut_off_h52w = len(uebit_h52w_leg_df_tickers)

    uebit_h52w_leg_df_tickers_cut_off = []
    uebit_h52w_leg_df_uebit_cut_off = []
    uebit_h52w_leg_df_h52w_cut_off = []
    uebit_h52w_leg_df_beta_cut_off = []
    # IF NON NAN and IN RANGE OF CUT_OFF
    for i in range(cut_off_h52w):
        if uebit_h52w_leg_df_sorted["h52w"][i] > 0 or uebit_h52w_leg_df_sorted["h52w"][i] < 0:
            uebit_h52w_leg_df_tickers_cut_off.append(uebit_h52w_leg_df_sorted["Ticker"][i])
            uebit_h52w_leg_df_uebit_cut_off.append(uebit_h52w_leg_df_sorted["uebit"][i])
            uebit_h52w_leg_df_h52w_cut_off.append(uebit_h52w_leg_df_sorted["h52w"][i])
            if uebit_h52w_leg_df_sorted["beta"][i] == "NM":
                uebit_h52w_leg_df_beta_cut_off.append(1)
            else:
                uebit_h52w_leg_df_beta_cut_off.append(uebit_h52w_leg_df_sorted["beta"][i])

    uebit_h52w_leg_df["Ticker"] = uebit_h52w_leg_df_tickers_cut_off
    uebit_h52w_leg_df["uebit"] = uebit_h52w_leg_df_uebit_cut_off
    uebit_h52w_leg_df["h52w"] = uebit_h52w_leg_df_h52w_cut_off
    uebit_h52w_leg_df["beta"] = uebit_h52w_leg_df_beta_cut_off

    return uebit_h52w_leg_df

def combination(in_sample, w_l_uebit, w_s_uebit, w_h52w, rtr, rtr_str, before_combination):
    ...
    if before_combination == []:
      ...
      w_h52w = w_h52w * 0.2 + before_combination[2] * 0.8
      rtr = rtr
      rtr_str = rtr_str

    for quarter in in_sample[-1:]:
        long_leg_df = createDataFrame(quarter=quarter, w_leg=w_l_uebit, w_h52w=w_h52w, true_if_long=True)
        short_leg_df = createDataFrame(quarter=quarter, w_leg=w_s_uebit, w_h52w=w_h52w, true_if_long=False)
    ...
    return geometric_mean, w_l_uebit, w_s_uebit, w_h52w, rtr_str, lever_ratio_mean

def liveTrade(quarter, w_l_uebit, w_s_uebit, w_h52w, rtr):
...
export_h52w_percentile = [] # from historic
...
for i in range(4, len(total_sample)): # from 4 historic periods
    ...
    for combi in combination_array:
        historic_result = combination(in_sample=in_sample, w_l_uebit=combi[0], w_s_uebit=combi[1], w_h52w=combi[2], rtr=combi[3], rtr_str=combi[4], before_combination=before_combination)
        ...
        if historic_result[0] > best_geometric_mean:
            ...
            # ^ w_l_uebit, w_s_uebit, w_h52w, rtr_str, lever_ratio_mean
    ...
    export_h52w_percentile.append(best_combination[2])
    ...
    live_result = liveTrade(quarter=live_quarter, w_l_uebit=best_combination[0], w_s_uebit=best_combination[1], w_h52w=best_combination[2], rtr=rtr_dict[best_combination[3]])
...
export["52WH Percentile"] = export_h52w_percentile
...
export.to_excel("results_UEBIT_52WH_RNN_NEW.xlsx")