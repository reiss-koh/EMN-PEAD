    ...
    for w_s_uebit in weight_short_uebit:
        for w_fh in weight_fh:
            for i in range(len(rtr_array)):
                combination_array.append([w_l_uebit, w_s_uebit, w_fh, rtr_array[i], rtr_array_str[i]])
...
def createDataFrame(quarter, w_leg, w_fh, true_if_long):
    ...
    for ticker in uebit_leg_df_tickers_cut_off:
        ...
        uebit_leg_df_fh_cut_off.append(fh[quarter][ticker])
    uebit_leg_df["Ticker"] = uebit_leg_df_tickers_cut_off
    uebit_leg_df["uebit"] = uebit_leg_df_uebit_cut_off
    uebit_leg_df["fh"] = uebit_leg_df_fh_cut_off
    uebit_leg_df["beta"] = uebit_leg_df_beta_cut_off
    uebit_leg_df.set_index("Ticker")

    # END OF UEBIT DF
    # START OF UEBIT FH DF

    uebit_fh_leg_df = pd.DataFrame()
    uebit_fh_leg_df_tickers = []
    uebit_fh_leg_df_fh = []

    uebit_fh_leg_df_sorted = uebit_leg_df.sort_values(by="fh", ascending=False)
    uebit_fh_leg_df_sorted.reset_index(drop=True, inplace=True)

    # GET RID OF NAN
    for i in range(len(uebit_fh_leg_df_sorted["Ticker"])):
        if uebit_fh_leg_df_sorted["fh"][i] > 0 or uebit_fh_leg_df_sorted["fh"][i] < 0:
            uebit_fh_leg_df_tickers.append(uebit_fh_leg_df_sorted["Ticker"][i])
            uebit_fh_leg_df_fh.append(uebit_fh_leg_df_sorted["fh"][i])

    # IDENTIFY CUTOFF POINT
    cut_off_fh = math.floor(w_fh * len(uebit_fh_leg_df_tickers))

    uebit_fh_leg_df_tickers_cut_off = []
    uebit_fh_leg_df_uebit_cut_off = []
    uebit_fh_leg_df_fh_cut_off = []
    uebit_fh_leg_df_beta_cut_off = []
    # IF NON NAN and IN RANGE OF CUT_OFF
    for i in range(cut_off_fh):
        if uebit_fh_leg_df_sorted["fh"][i] > 0 or uebit_fh_leg_df_sorted["fh"][i] < 0:
            uebit_fh_leg_df_tickers_cut_off.append(uebit_fh_leg_df_sorted["Ticker"][i])
            uebit_fh_leg_df_uebit_cut_off.append(uebit_fh_leg_df_sorted["uebit"][i])
            uebit_fh_leg_df_fh_cut_off.append(uebit_fh_leg_df_sorted["fh"][i])
            if uebit_fh_leg_df_sorted["beta"][i] == "NM":
                uebit_fh_leg_df_beta_cut_off.append(1)
            else:
                uebit_fh_leg_df_beta_cut_off.append(uebit_fh_leg_df_sorted["beta"][i])

    uebit_fh_leg_df["Ticker"] = uebit_fh_leg_df_tickers_cut_off
    uebit_fh_leg_df["uebit"] = uebit_fh_leg_df_uebit_cut_off
    uebit_fh_leg_df["fh"] = uebit_fh_leg_df_fh_cut_off
    uebit_fh_leg_df["beta"] = uebit_fh_leg_df_beta_cut_off

    return uebit_fh_leg_df

def combination(in_sample, w_l_uebit, w_s_uebit, w_fh, rtr, rtr_str, before_combination):
    ...
    for quarter in in_sample[-1:]:
        long_leg_df = createDataFrame(quarter=quarter, w_leg=w_l_uebit, w_fh=w_fh, true_if_long=True)
        short_leg_df = createDataFrame(quarter=quarter, w_leg=w_s_uebit, w_fh=w_fh, true_if_long=False)
    ...
    return geometric_mean, w_l_uebit, w_s_uebit, w_fh, rtr_str, lever_ratio_mean

def liveTrade(quarter, w_l_uebit, w_s_uebit, w_fh, rtr):
...
for i in range(4, len(total_sample)): # from 4 historic periods
    ...
    for combi in combination_array:
        historic_result = combination(in_sample=in_sample, w_l_uebit=combi[0], w_s_uebit=combi[1], w_fh=combi[2], rtr=combi[3], rtr_str=combi[4], before_combination=before_combination)
        ...
        if historic_result[0] > best_geometric_mean:
            ...
            # ^ w_l_uebit, w_s_uebit, w_fh, rtr_str, lever_ratio_mean
    ...
    live_result = liveTrade(quarter=live_quarter, w_l_uebit=best_combination[0], w_s_uebit=best_combination[1], w_fh=best_combination[2], rtr=rtr_dict[best_combination[3]])
...
export.to_excel("results_UEBIT_FH_RNN_anti.xlsx")
