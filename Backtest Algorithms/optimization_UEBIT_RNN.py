...
for w_l_uebit in weight_long_uebit:
    for w_s_uebit in weight_short_uebit:
        for i in range(len(rtr_array)):
            combination_array.append([w_l_uebit, w_s_uebit, rtr_array[i], rtr_array_str[i]])
...
def createDataFrame(quarter, w_leg, true_if_long):
    ...
    for ticker in uebit_leg_df_tickers_cut_off:
        if beta[quarter][ticker] == "NM":
            uebit_leg_df_beta_cut_off.append(1)
        else:
...
def combination(in_sample, w_l_uebit, w_s_uebit, rtr, rtr_str, before_combination):
    ...
    for quarter in in_sample[-1:]:
        long_leg_df = createDataFrame(quarter=quarter, w_leg=w_l_uebit, true_if_long=True)
        short_leg_df = createDataFrame(quarter=quarter, w_leg=w_s_uebit, true_if_long=False)
    ...
    return geometric_mean, w_l_uebit, w_s_uebit, rtr_str, lever_ratio_mean

def liveTrade(quarter, w_l_uebit, w_s_uebit, rtr):
...
for i in range(4, len(total_sample)): # from 4 historic periods
    ...
    for combi in combination_array:
        historic_result = combination(in_sample=in_sample, w_l_uebit=combi[0], w_s_uebit=combi[1], rtr=combi[2], rtr_str=combi[3], before_combination=before_combination)
        ...
        if historic_result[0] > best_geometric_mean:
            ...
            # w_l_uebit, w_s_uebit, rtr_str, lever_ratio_mean
    ...
    live_result = liveTrade(quarter=live_quarter, w_l_uebit=best_combination[0], w_s_uebit=best_combination[1], rtr=rtr_dict[best_combination[2]])
...
export.to_excel("results_UEBIT_RNN.xlsx")
