...
rtr_array = (rtr30, rtr40, rtr50)  # based on domain knowledge we excl. rtr5, rtr10, rtr20
...
for w_l_suebit in weight_long_suebit:
    for w_s_suebit in weight_short_suebit:
        for i in range(len(rtr_array)):
            combination_array.append([w_l_suebit, w_s_suebit, rtr_array[i], rtr_array_str[i]])

def createDataFrame(quarter, w_leg, true_if_long):
    ...
    for ticker in suebit_leg_df_tickers_cut_off:
        if beta[quarter][ticker] == "NM":
            suebit_leg_df_beta_cut_off.append(1)
        else:
    ...
    return suebit_leg_df

def combination(in_sample, w_l_suebit, w_s_suebit, rtr, rtr_str, before_combination):
    ...
    for quarter in in_sample[-1:]:
        long_leg_df = createDataFrame(quarter=quarter, w_leg=w_l_suebit, true_if_long=True)
        short_leg_df = createDataFrame(quarter=quarter, w_leg=w_s_suebit, true_if_long=False)
    ...
    return geometric_mean, w_l_suebit, w_s_suebit, rtr_str, lever_ratio_mean

def liveTrade(quarter, w_l_suebit, w_s_suebit, rtr):
...
for i in range(4, len(total_sample)): # from 4 historic periods
    ...
    for combi in combination_array:
        historic_result = combination(in_sample=in_sample, w_l_suebit=combi[0], w_s_suebit=combi[1], rtr=combi[2], rtr_str=combi[3], before_combination=before_combination)
        print(historic_result)
        if historic_result[0] > best_geometric_mean:
            best_geometric_mean = historic_result[0]
            best_combination = [historic_result[1], historic_result[2], historic_result[3], historic_result[4]]
            # w_l_suebit, w_s_suebit, rtr_str, lever_ratio_mean
    ...
    export_holding_period.append(best_combination[2])
    export_mean_lever_ratio_historic.append(best_combination[3])
    ...
    live_result = liveTrade(quarter=live_quarter, w_l_suebit=best_combination[0], w_s_suebit=best_combination[1], rtr=rtr_dict[best_combination[2]])
...
export.to_excel("results_SUEBIT_RNN.xlsx")
