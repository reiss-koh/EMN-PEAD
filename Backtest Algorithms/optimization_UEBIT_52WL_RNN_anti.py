...
l52w = pd.read_excel('ALL_52WL_READY2.xlsx', index_col=0)  # set stock code as index
...
weight_l52w = (0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8)
combination_array = []

for w_l_uebit in weight_long_uebit:
    for w_s_uebit in weight_short_uebit:
        for w_l52w in weight_l52w:
            for i in range(len(rtr_array)):
                combination_array.append([w_l_uebit, w_s_uebit, w_l52w, rtr_array[i], rtr_array_str[i]])

def createDataFrame(quarter, w_leg, w_l52w, true_if_long):
    ...
    for ticker in uebit_leg_df_tickers_cut_off:
        ...
        uebit_leg_df_l52w_cut_off.append(l52w[quarter][ticker])
    uebit_leg_df["Ticker"] = uebit_leg_df_tickers_cut_off
    uebit_leg_df["uebit"] = uebit_leg_df_uebit_cut_off
    uebit_leg_df["l52w"] = uebit_leg_df_l52w_cut_off
    uebit_leg_df["beta"] = uebit_leg_df_beta_cut_off
    uebit_leg_df.set_index("Ticker")

    # END OF UEBIT DF
    # START OF UEBIT l52w DF

    uebit_l52w_leg_df = pd.DataFrame()
    uebit_l52w_leg_df_tickers = []
    uebit_l52w_leg_df_l52w = []

    uebit_l52w_leg_df_sorted = uebit_leg_df.sort_values(by="l52w", ascending=true_if_long)
    uebit_l52w_leg_df_sorted.reset_index(drop=True, inplace=True)

    # GET RID OF NAN
    for i in range(len(uebit_l52w_leg_df_sorted["Ticker"])):
        if uebit_l52w_leg_df_sorted["l52w"][i] > 0 or uebit_l52w_leg_df_sorted["l52w"][i] < 0:
            uebit_l52w_leg_df_tickers.append(uebit_l52w_leg_df_sorted["Ticker"][i])
            uebit_l52w_leg_df_l52w.append(uebit_l52w_leg_df_sorted["l52w"][i])

    # IDENTIFY CUTOFF POINT
    cut_off_l52w = math.floor(w_l52w * len(uebit_l52w_leg_df_tickers))

    uebit_l52w_leg_df_tickers_cut_off = []
    uebit_l52w_leg_df_uebit_cut_off = []
    uebit_l52w_leg_df_l52w_cut_off = []
    uebit_l52w_leg_df_beta_cut_off = []
    # IF NON NAN and IN RANGE OF CUT_OFF
    for i in range(cut_off_l52w):
        if uebit_l52w_leg_df_sorted["l52w"][i] > 0 or uebit_l52w_leg_df_sorted["l52w"][i] < 0:
            uebit_l52w_leg_df_tickers_cut_off.append(uebit_l52w_leg_df_sorted["Ticker"][i])
            uebit_l52w_leg_df_uebit_cut_off.append(uebit_l52w_leg_df_sorted["uebit"][i])
            uebit_l52w_leg_df_l52w_cut_off.append(uebit_l52w_leg_df_sorted["l52w"][i])
            if uebit_l52w_leg_df_sorted["beta"][i] == "NM":
                uebit_l52w_leg_df_beta_cut_off.append(1)
            else:
                uebit_l52w_leg_df_beta_cut_off.append(uebit_l52w_leg_df_sorted["beta"][i])

    uebit_l52w_leg_df["Ticker"] = uebit_l52w_leg_df_tickers_cut_off
    uebit_l52w_leg_df["uebit"] = uebit_l52w_leg_df_uebit_cut_off
    uebit_l52w_leg_df["l52w"] = uebit_l52w_leg_df_l52w_cut_off
    uebit_l52w_leg_df["beta"] = uebit_l52w_leg_df_beta_cut_off

    return uebit_l52w_leg_df

def combination(in_sample, w_l_uebit, w_s_uebit, w_l52w, rtr, rtr_str, before_combination):
    ...
    if before_combination == []:
      ...
      w_l52w = w_l52w * 0.2 + before_combination[2] * 0.8
      rtr = rtr
      rtr_str = rtr_str

    for quarter in in_sample[-1:]:
        long_leg_df = createDataFrame(quarter=quarter, w_leg=w_l_uebit, w_l52w=w_l52w, true_if_long=True)
        short_leg_df = createDataFrame(quarter=quarter, w_leg=w_s_uebit, w_l52w=w_l52w, true_if_long=False)
    ...
    return geometric_mean, w_l_uebit, w_s_uebit, w_l52w, rtr_str, lever_ratio_mean

def liveTrade(quarter, w_l_uebit, w_s_uebit, w_l52w, rtr):
...
export_l52w_percentile = [] # from historic
...
for i in range(4, len(total_sample)): # from 4 historic periods
    ...
    for combi in combination_array:
        historic_result = combination(in_sample=in_sample, w_l_uebit=combi[0], w_s_uebit=combi[1], w_l52w=combi[2], rtr=combi[3], rtr_str=combi[4], before_combination=before_combination)
        ...
        if historic_result[0] > best_geometric_mean:
            ...
            # ^ w_l_uebit, w_s_uebit, w_l52w, rtr_str, lever_ratio_mean
    ...
    export_l52w_percentile.append(best_combination[2])
    ...
    live_result = liveTrade(quarter=live_quarter, w_l_uebit=best_combination[0], w_s_uebit=best_combination[1], w_l52w=best_combination[2], rtr=rtr_dict[best_combination[3]])
...
export["52WH Percentile"] = export_l52w_percentile
...
export.to_excel("results_UEBIT_52WL_RNN_anti.xlsx")